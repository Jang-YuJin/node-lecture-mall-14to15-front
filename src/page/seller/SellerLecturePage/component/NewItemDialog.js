...
const NewItemDialog = ({ mode, showDialog, setShowDialog }) => {
  ...
  useEffect(() => {
    ...
    if (showDialog) {
      if (mode === "edit") {
        ...
        const sizeArray = Object.keys(selectedLecture.stock).map((size) => [
          size,
          selectedLecture.stock[size],
        ]);
        setLectureVideo(sizeArray);
      } else {
  ...
  const handleSubmit = (event) => {
    ...
    if (imgError || lectureVideoError || fileTxtbkError) {
      return;
    }
